...
public:
    ...
    void startXCS();
    void singleStepExp();
    void singleStepExplore(char pczState[], int nInsts);
    void singleStepExploit(char pczState[], int nInsts, int arrCorrect[], double arrSysErr[]);
    void writePerformance(int arrCorrect[], double arrSysErr[], int nInsts, ClfrSetType eClfrSetType, LogFileType eFileType);
    void writePerformanceHeader(int nExpNo, LogFileType eFileType);
